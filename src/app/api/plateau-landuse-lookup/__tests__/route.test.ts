...
describe('/api/plateau-landuse-lookup', () => {
  ...
  test('adds subdivided candidates for oversized landuse polygons', async () => {
    (fetchZipEntry as jest.Mock).mockResolvedValueOnce(new Uint8Array([1, 2, 3]));
    const feature = {
      extent: 4096,
      properties: { class: 'large' },
    ...
    const req = new Request('http://localhost/api/plateau-landuse-lookup', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ lat: 35.6332, lng: 139.7132 }),
    });
    ...
    const body = await res.json();
    const subdivisionCandidates = body.candidates.filter(
      (candidate: { candidateKind?: string }) => candidate.candidateKind === 'subdivision',
    );
    const originalCandidates = body.candidates.filter(
      (candidate: { candidateKind?: string }) => candidate.candidateKind === 'original',
    );

    expect(subdivisionCandidates.length).toBeGreaterThan(0);
    expect(originalCandidates.length).toBeGreaterThan(0);
    expect(
      subdivisionCandidates.every(
        (candidate: { area?: number; parentArea?: number | null }) =>
          typeof candidate.area === 'number' &&
          typeof candidate.parentArea === 'number' &&
          candidate.area < candidate.parentArea,
      ),
    ).toBe(true);
  });
});
