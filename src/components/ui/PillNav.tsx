...
export default function PillNav({
  ...
  const LogoContent = logo ? (
    ...
    <span style={{ fontSize: 12, fontWeight: 700, lineHeight: 1 }}>{logoText ?? 'VC'}</span>
  );

  return (
    <div className="pill-nav-container">
      <nav className={`pill-nav ${className}`} aria-label="Primary" style={cssVars}>
        {isAnchorOrExternal(logoHref) ? (
          <a className="pill-logo" href={logoHref} aria-label="Home" onMouseEnter={handleLogoEnter} ref={logoRef}>
            {LogoContent}
          </a>
        ) : (
          <Link className="pill-logo" href={logoHref} aria-label="Home" onMouseEnter={handleLogoEnter} ref={logoRef}>
            {LogoContent}
          </Link>
        )}
