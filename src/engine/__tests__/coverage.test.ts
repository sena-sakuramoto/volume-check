...
describe('calculateMaxCoverage', () => {
  ...
  it('caps effective ratio at 100% (1.0)', () => {
    ...
    // 150 * 1.0 = 150
    expect(calculateMaxCoverage(site, zoning, site.area)).toBe(150);
  });
  ...
  it('works with a small site area', () => {
    ...
    const zoning = makeZoning({ coverageRatio: 0.6 });
    // 25 * 0.6 = 15
    expect(calculateMaxCoverage(smallSite, zoning, smallSite.area)).toBe(15);
  });
  ...
  it('returns proper decimal result for non-round areas', () => {
    ...
    const zoning = makeZoning({ coverageRatio: 0.6 });
    // 77 * 0.6 = 46.2
    expect(calculateMaxCoverage(oddSite, zoning, oddSite.area)).toBeCloseTo(46.2, 2);
  });

  it('handles coverageRatio of 0.3', () => {
    const zoning = makeZoning({ coverageRatio: 0.3 });
    // 150 * 0.3 = 45
    expect(calculateMaxCoverage(site, zoning, site.area)).toBe(45);
  });

  it('clamps coverage to buildable area', () => {
    const zoning = makeZoning({ coverageRatio: 0.8 });
    expect(calculateMaxCoverage(site, zoning, 100)).toBe(100);
  });
});
